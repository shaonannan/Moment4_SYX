...
class RecurrentPopulation(object):
    ...
    def initialize_nmdajump(self):
        ...
        for c in self.source_connection_list:
            if(c.conn_type == 'LongRange'):
                try:
                    curr_nmdajump = self.nmdajump_dict.setdefault(c.connection_distribution,0)
                    curr_nmdakeep = self.nmdakeep_dict.setdefault(c.connection_distribution,0)
                except:
                    c.initialize_connection_ditribution()
                    curr_nmdajump = self.nmdajump_dict.setdefault(C.connection_distribution,0)
                    curr_nmdakeep = self.nmdakeep_dict.setdefault(C.connection_distribution,0)
                self.nmdajump_dict[c.connection_distribution] = curr_nmdajump + (etd-etr) * trise/(tdamp-trise) # c.curr_Inmda * etd + 
                self.nmdakeep_dict[c.connection_distribution] = curr_nmdakeep #* etd 
                self.nmda_dict[c.connection_distribution] = self.nmdakeep_dict[c.connection_distribution] + self.nmdajump_dict[c.connection_distribution]

    def update_total_nmdajump_dict(self):
        """
        long-range connections lift base-voltage, but didn't generate flux!!!
        """
        deltat = self.dt
        ...
        for c in self.source_connection_list:
            if(c.conn_type == 'LongRange'):
                self.nmdajump_dict[c.connection_distribution] = 0.0
                self.nmdakeep_dict[c.connection_distribution] = 0.0
                self.nmdajump_dict[c.connection_distribution] += (etd-etr) * trise/(tdamp-trise)
                self.nmdakeep_dict[c.connection_distribution] += c.curr_Inmda *self.dt # * etd
                self.nmdajump_dict[c.connection_distribution] = self.nmdajump_dict[c.connection_distribution] * c.weights
                self.nmdakeep_dict[c.connection_distribution] = self.nmdakeep_dict[c.connection_distribution] * c.weights
                self.nmda_dict[c.connection_distribution] = self.nmdajump_dict[c.connection_distribution] + self.nmdakeep_dict[c.connection_distribution]
                c.connection_distribution.threshold_fluxn_vector, c.connection_distribution.fluxn_matrix = flux_matrix(self.edges,self.nmdajump_dict[c.connection_distribution],1.0)
                c.connection_distribution.threshold_fluxk_vector, c.connection_distribution.fluxk_matrix = flux_matrix(self.edges,self.nmdakeep_dict[c.connection_distribution],1.0)
                # c.connection_distribution.threshold_fluxn_vector, c.connection_distribution.fluxn_matrix = flux_matrix(self.edges,self.nmda_dict[c.connection_distribution],1.0)

                """
                print 'nmda jump:' ,self.nmdajump_dict[c.connection_distribution],'nmda keep:' ,self.nmdakeep_dict[c.connection_distribution]
                print 'Inmda: ',c.curr_Inmda
                print 'diff: ',(tdamp-trise)
                print 'diff 1:',(etd -etr)
                if self.nmdajump_dict[c.connection_distribution]<0.0:
                    pause
                """
    ...
    def update_total_flux_matrix(self):
        ...
        """
        # update NMDA
        self.update_total_nmdajump_dict()
        ...
        for key,val in self.total_inputlr_dict.items():
            # print 'LR val: ',val
            try:
                total_flux_matrix += key.fluxn_matrix * val
            except:
                key.initialize()
                total_flux_matrix += key.fluxn_matrix * val
        """
    ...
    def update_firing_rate(self):
        ...
        if (flag==1):
            """
            fluxn_vector = reduce(np.add,[key.threshold_fluxn_vector * val for key,val in
                                    self.total_inputlr_dict.items()])
            # adding together
            flux_vector += fluxn_vector
            """
